...
class _LoginScreenState extends ConsumerState<LoginScreen> {
  ...
  void initState() {
    ...
    if (PlatformUtils.isWeb) {
      debugLog(
        '🔍 [INIT] PlatformUtils.isWeb = true, leyendo parámetros de URL',
      );
      try {
        final uri = Uri.base;
        ...
        final age18Param =
            uri.queryParameters['age18'] ?? uri.queryParameters['age'];
        final rulesParam =
            uri.queryParameters['rules'] ??
            uri.queryParameters['reglas'] ??
            uri.queryParameters['normas'];
        final guestParam =
            uri.queryParameters['guest'] ?? uri.queryParameters['invitado'];
        ...
        if (guestParam != null) {
          ...
          if (v == 'true' || v == '1' || v == 'yes') {
            ...
            debugLog(
              '🔍 [URL] ✅ guest/invitado=1 (Uri.base): entrar como invitado por parámetro',
            );
          }
        }
        ...
        if (!_guestFromUrl &&
            nickParam != null &&
            nickParam.trim().isNotEmpty) {
          final sanitized = _sanitizeNick(nickParam);
          if (sanitized.isNotEmpty) {
            urlNick = sanitized;
            debugLog(
              '🔍 [URL] ✅ Nick leído de Uri.base: "$nickParam" -> Sanitizado: "$urlNick"',
            );
          } else {
            debugLog(
              '🔍 [URL] ⚠️ Nick inválido tras sanitizar (Uri.base): "$nickParam"',
            );
          }
        }
        ...
        if (autoJoinParam != null) {
          ...
          autoJoin =
              ...
              autoJoinValue == 'yes';
          debugLog(
            '🔍 [URL] ✅ autoJoin leído de Uri.base: "$autoJoinParam" -> autoJoin=$autoJoin',
          );
        }
        ...
        if (joinOficialParam != null) {
          ...
          debugLog(
            '🔍 [URL] ✅ joinchanneloficial leído de Uri.base: "$joinOficialParam" -> $joinChannelOficialFromUrl',
          );
        }
        ...
        debugLog(
          '🔍 [URL] Parámetros finales - nick: $urlNick, channel: $urlChannel, autojoin: $autoJoin, joinchanneloficial: $joinChannelOficialFromUrl, geolocation: $_geolocationEnabled, age18: $_urlAge18Validated, guest: $_guestFromUrl',
        );
    ...
    final random = Random();
    final randomNumber =
        random.nextInt(90000) + 10000; // Número entre 10000 y 99999
    ...
    debugLog(
      '🔍 [LOGIN] NickController inicializado con: "${_nickController.text}"',
    );
    ...
    if (_selectedServer == null) {
      ...
      if (sslServers.isNotEmpty) {
        ...
        debugLog(
          '🔍 [LOGIN] Servidor inicial aleatorio seleccionado: ${_selectedServer!.host}:${_selectedServer!.port}',
        );
      }
    }
    ...
    if (PlatformUtils.isWeb && joinChannelOficialFromUrl != null) {
      ...
      debugLog(
        '🌐 [LOGIN] joinchanneloficial aplicado al IRCService: $joinChannelOficialFromUrl',
      );
    }
    ...
    if (_urlChannel != null && _urlChannel!.trim().isNotEmpty) {
      ...
      debugLog(
        '🔍 [URL] Verificación - _channelController.text = "${_channelController.text}"',
      );
    } else {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      if (PlatformUtils.isWeb && _geolocationEnabled) {
        try {
          ...
          if (loc != null && mounted) {
            ...
            final channelCity = GeoIPService.cityRegionToChannelName(
              city,
              region,
            );
            final channelCountry = country != null && country.isNotEmpty
                ...
                : null;
            final hasUrlChannel =
                _urlChannel != null && _urlChannel!.trim().isNotEmpty;
            ...
            final geoChannels = <String>[
              if (channelCountry != null && channelCountry != channelCity)
                channelCountry,
              channelCity,
            ];
            if (!hasUrlChannel &&
                (currentChannelStored == null ||
                    currentChannelStored.isEmpty) &&
                autoJoinChannels.isEmpty) {
              ...
              debugLog(
                '🌍 [GEOIP] Canales por país/ciudad (sin canal previo): $geoChannels',
              );
            } else {
              ...
              final mainNorm = mainChannel.isNotEmpty
                  ? (mainChannel.startsWith('#')
                        ? mainChannel
                        : '#$mainChannel')
                  : null;
              final toJoin = <String>[];
              ...
              debugLog(
                '🌍 [GEOIP] Añadido join a canales país y ciudad/región: $geoChannels (canales: $toJoin)',
              );
            }
          }
        } catch (e) {
      ...
      debugLog(
        '🔍 [LOGIN] Estado inicial - servidor: ${selectedServerProfile?.name ?? "null"}',
      );
      debugLog(
        '🔍 [LOGIN] Servidor host: ${selectedServerProfile?.host ?? "null"}, port: ${selectedServerProfile?.port ?? "null"}',
      );
      ...
      final hasAutoJoinData =
          currentNick != null &&
          currentNick.isNotEmpty &&
          (currentChannel != null && currentChannel.isNotEmpty ||
              autoJoinChannels.isNotEmpty);

      if (selectedServerProfile != null) {
        debugLog(
          '🔍 [LOGIN] ✅ Servidor seleccionado desde provider: ${selectedServerProfile.name}',
        );
        debugLog(
          '🔍 [LOGIN] Servidor host: ${selectedServerProfile.host}, port: ${selectedServerProfile.port}',
        );
        ...
        debugLog(
          '🔍 [LOGIN] Campos actualizados - host: ${_hostController.text}, port: ${_portController.text}',
        );
        ...
        if (_urlChannel == null || _urlChannel!.isEmpty) {
          if (currentChannel != null && currentChannel.isNotEmpty) {
            ...
            debugLog(
              '🔍 [LOGIN] Usando primer canal de autojoin: ${autoJoinChannels.first}',
            );
          }
        } else {
          debugLog(
            '🔍 [LOGIN] Canal de URL tiene prioridad, no se sobrescribe con provider',
          );
        }
        ...
        if ((urlNick == null || urlNick.isEmpty) &&
          ...
          debugLog(
            '🔍 [LOGIN] Nick de URL tiene prioridad, no se sobrescribe con provider',
          );
        }
        ...
        if (hasAutoJoinData && !(autoJoin && PlatformUtils.isWeb)) {
          debugLog(
            '🔍 [AUTOJOIN] ✅ Condiciones cumplidas para autojoin: servidor=${selectedServerProfile.name}, nick=$currentNick, canales=$autoJoinChannels',
          );
          ...
          Future.delayed(const Duration(milliseconds: 500), () async {
            ...
            debugLog(
              '🔍 [AUTOJOIN] Verificando campos: host=$host, port=$port, nick=$nick, channel=$channel',
            );

            if (host.isNotEmpty && nick.isNotEmpty && channel.isNotEmpty) {
              debugLog(
                '🔍 [AUTOJOIN] ✅ Todos los campos están completos, iniciando conexión...',
              );
              debugLog(
                '🔍 [AUTOJOIN] Auto-uniéndose después de cambiar servidor: host=$host, port=$port, nick=$nick, channel=$channel',
              );
              ...
              try {
                await _connect();
                debugLog(
                  '🔍 [AUTOJOIN] ✅ Conexión exitosa después de cambiar servidor',
                );
              } catch (e) {
              ...
              debugLog(
                '🔍 [AUTOJOIN] ⚠️ Campos incompletos - host: ${host.isNotEmpty}, nick: ${nick.isNotEmpty}, channel: ${channel.isNotEmpty}',
              );
            }
          });
        } else {
        ...
        debugLog(
          '🔍 [LOGIN] No hay servidor seleccionado en el provider, usando selección por GeoIP',
        );

        // Si autojoin está activado desde URL y no hay servidor seleccionado, hacer autojoin
        if (autoJoin &&
            PlatformUtils.isWeb &&
            (urlNick != null || _urlChannel != null)) {
          debugLog(
            '🔍 [AUTOJOIN_URL] ✅ Autojoin activado desde URL - nick: $urlNick, channel: $_urlChannel',
          );
          ...
          if (channelToUse.isNotEmpty) {
            ...
            if (sslServers.isNotEmpty) {
              ...
              if (mounted) {
                ...
                if (host.isNotEmpty && nick.isNotEmpty && channel.isNotEmpty) {
                  debugLog(
                    '🔍 [AUTOJOIN_URL] Intentando conectar: host=$host, port=$port, nick=$nick, channel=$channel',
                  );
              ...
              if (_geolocationEnabled) {
                ...
                if (sslServers.isNotEmpty) {
                  final random = Random();
                  _selectedServer =
                      sslServers[random.nextInt(sslServers.length)];
                  _updateServerFields(_selectedServer!);
                } else {
                  await _selectServerByGeoIP();
                }
    ...
    try {
      ...
      if (isAmericas == true) {
        ...
        if (nonAmericasServers.isNotEmpty) {
          ...
          if (PlatformUtils.isWeb) {
            debugLog(
              '🌎 [GEOIP] Usuario fuera de América, usando ${selectedServer.host}',
            );
          }
        } else {
          ...
          if (PlatformUtils.isWeb) {
            debugLog(
              '🌎 [GEOIP] No hay servidores disponibles, usando por defecto',
            );
          }
        }
      } else {
        ...
        if (PlatformUtils.isWeb) {
          debugLog(
            '🌎 [GEOIP] No se pudo determinar ubicación, usando selección aleatoria',
          );
        }
      }
    } catch (e) {
      ...
      if (PlatformUtils.isWeb) {
        debugLog(
          '🌎 [GEOIP] Error al detectar ubicación: $e, usando selección aleatoria',
        );
      }
      final random = Random();
      final selectedIndex = random.nextInt(sslServers.length);
  ...
  Future<void> _loadLoginPrefs() async {
    ...
    try {
      ...
      if (_identifyWithNick &&
        ...
        if (savedPasswordNick != null &&
            savedPasswordNick.trim() == savedNick.trim()) {
          final savedPassword = prefs.getString(_prefIdentifyPassword);
          if (savedPassword != null && savedPassword.isNotEmpty) {
            _passwordController.text = savedPassword;
            debugLog(
              '🔍 [LOGIN] Contraseña de identificación restaurada para nick: $savedNick',
            );
          }
        }
      }
      final savedAutoReconnectEnabled = prefs.getBool(
  ...
  Future<void> _saveLoginPrefs(
    String nick,
    String channel,
    bool identifyWithNick, {
  ...
  Future<void> _loadChannels() async {
    ...
    try {
      final response = await http
          .get(Uri.parse('https://canales.globalchat.org/proxy/channels.php'))
      ...
      if (response.statusCode == 200) {
        ...
        if (data['channels'] != null) {
          ...
          setState(() {
            _channels =
                channelsJson.map((json) => ChannelInfo.fromJson(json)).where((
                  channel,
                ) {
                  ...
                  return !_prohibitedChannels.any(
                    (prohibited) =>
                        channelNameLower == prohibited.toLowerCase(),
                  );
                }).toList()..sort(
                  (a, b) => b.users.compareTo(a.users),
                ); // Ordenar por usuarios (mayor a menor)
  ...
  Future<void> _applyGeoIpChannelsFromToggle() async {
    ...
    try {
      ...
      String mainChannel =
          currentChannelText.isEmpty || currentChannelText == '#'
          ? channelCity
          : currentChannelText;
      ...
      debugLog(
        '🌍 [GEOIP] (toggle) Canales sugeridos: $geoChannels (canales: $toJoin)',
      );
    } catch (e) {
  ...
  void _clearGeoIpChannelsFromToggle() {
    ...
    debugLog(
      '🌍 [GEOIP] (toggle) Desactivado: canal y autojoin limpiados para selección manual',
    );
  }
  ...
  Future<void> _connect() async {
    if (!_confirmOver14) {
      setState(() {
        _errorMessage =
            'Debes confirmar que eres mayor de 14 años para continuar.';
        _isLoading = false;
        _isAutoJoining = false;
      });
      return;
    ...
    if (!_acceptRules) {
      setState(() {
        _errorMessage =
            'Debes aceptar las reglas del canal/red para continuar.';
        _isLoading = false;
        _isAutoJoining = false;
      });
      return;
    ...
    if (_nickHasInvalidCharacters(_nickController.text)) {
      setState(() {
        _errorMessage =
            'El nick no puede contener ñ, acentos, emojis ni otros caracteres especiales. '
    ...
    try {
      ...
      final identifyPasswordForRecovery = _identifyWithNick
      ...
      if (autoJoinChannels.isNotEmpty) {
        ...
        debugLog(
          '🔍 [CONNECT] Usando canal del formulario: $normalizedChannel',
        );
      }
      ...
      if (mounted) {
        final identifyPassword = _identifyWithNick
            ? _passwordController.text.trim()
            : null;
      ...
      if (e.toString().contains('SocketException') ||
        ...
        errorMessage =
            'Error SSL/TLS. Verifica que el servidor soporte conexiones seguras en el puerto 6697.';
      }
