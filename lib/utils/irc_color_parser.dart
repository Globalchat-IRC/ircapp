...
class IRCColorParser {
  ...
  static List<TextSpan> parseIRCMessage(String message, {Color? defaultColor}) {
    ...
    while (i < message.length) {
      ...
      if (char == '\x03') {
        ...
        String? colorCode;
        if (i < message.length &&
            message[i].codeUnitAt(0) >= 48 &&
            message[i].codeUnitAt(0) <= 57) {
          colorCode = message[i];
          i++;
  ...
  static String stripIRCFormatting(String message, {bool aggressive = false}) {
    ...
    cleaned = cleaned.replaceAll(
      RegExp(r'[\x00-\x08\x0B-\x0C\x0E-\x1A\x1C\x1E]'),
      '',
    );
    ...
    if (aggressive) {
      ...
      cleaned = cleaned.replaceAll(
        RegExp(r'\b(\d)([A-Za-zÁÉÍÓÚáéíóúÑñ])'),
        r'$2',
      );
